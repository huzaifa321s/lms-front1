...
import { useParams, createLazyFileRoute, useNavigate } from '@tanstack/react-router'
import {
  ArrowLeft,
  BookOpen,
  User,
...
export const Route = createLazyFileRoute('/_authenticated/admin/courses/$courseID')(
  {
    loader: ({ params }) => {
...
function RouteComponent() {
  ...
  {courseObj?.material?.map((material, idx) => (
    <AccordionItem
      key={idx}
      value={`material-${idx}`}
      className="rounded-[12px] border border-[#e2e8f0] bg-white shadow-sm hover:shadow-md transition-all duration-300"
    >
      {/* Accordion Header */}
      <AccordionTrigger className="px-6 py-4 hover:no-underline [&[data-state=open]>div]:text-[#2563eb]">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-semibold text-[#1e293b]">{material.title}</h3>
        </div>
      </AccordionTrigger>
      ...
      <AccordionContent className="px-6 pb-6">
        <div className="space-y-4">
          ...
          {material.description && (
            <p className="text-sm text-[#64748b]">{material.description}</p>
          )}
          ...
          {material.media && (
            <>
              ...
              {/* Images */}
              {material.media.match(/\.(png|jpg|jpeg|gif)$/i) && (
