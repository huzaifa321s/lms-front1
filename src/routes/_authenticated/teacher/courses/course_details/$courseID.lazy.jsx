...
import { useParams, useRouterState, createLazyFileRoute } from '@tanstack/react-router'
import {
  ArrowLeft,
  BookOpen,
  User,
...
const MaterialPreview = ({ media }) => {
  ...
  if (media.endsWith(".pdf")) {
    return (
      <div className="mt-3 rounded-xl border bg-slate-50 h-[70vh] overflow-y-auto">
        <iframe
          src={url}
  ...
  if (media.match(/\.(png|jpg|jpeg|gif)$/i)) {
    return (
      <div className="mt-3 flex items-center justify-center rounded-xl border bg-slate-100 h-[70vh]">
        <img
          src={url}
  ...
  if (media.match(/\.(mp4|webm|ogg)$/i)) {
    return (
      <div className="mt-3 rounded-xl border bg-black h-[70vh] flex items-center justify-center">
        <video
          controls
          src={url}
